...
struct Convertor *
convertor_init(int64_t src_ch_layout, enum AVSampleFormat src_sp_fmt, int src_sp_rate,
    ...
    convertor->src_nb_buffers = av_sample_fmt_is_planar(convertor->src_sample_fmt)
                                ? convertor->src_nb_channels : 1;

    convertor->dst_nb_channels = av_get_channel_layout_nb_channels(dst_ch_layout);
    convertor->dst_nb_buffers = av_sample_fmt_is_planar(convertor->dst_sample_fmt)
                                ? convertor->dst_nb_channels : 1;

    LOGE("%d, %d, %d, %d, %d", src_sp_rate, dst_sp_rate, src_sp_fmt, dst_sp_fmt,
         AV_SAMPLE_FMT_S16P);
    LOGE("%d, %d", convertor->src_nb_buffers, convertor->dst_nb_buffers);
    // init
    ...
    int tmp_ret = av_samples_alloc_array_and_samples(&convertor->src_buffers,
                                                     &convertor->src_linesize,
                                                     convertor->src_nb_channels,
                                                     convertor->src_nb_samples,
                                                     convertor->src_sample_fmt, 0);
    if (tmp_ret < 0) {
    ...
    convertor->max_dst_nb_samples = convertor->dst_nb_samples =
            (int) av_rescale_rnd(convertor->src_nb_samples, convertor->dst_sample_rate,
                                 convertor->src_sample_rate, AV_ROUND_UP);

    tmp_ret = av_samples_alloc_array_and_samples(&convertor->dst_buffers, &convertor->dst_linesize,
                                                 convertor->dst_nb_channels,
                                                 convertor->dst_nb_samples,
                                                 convertor->dst_sample_fmt, 0);
    if (tmp_ret < 0) {
...
int convertor_feed_data(struct Convertor *convertor, uint8_t **data, int len) {
    ...
    convertor->src_nb_samples = len / convertor->src_nb_channels / 2;
    ...
    convertor->dst_nb_samples = (int) av_rescale_rnd(
            ...
            AV_ROUND_UP);
    LOGE("convertor_feed_data : %d, %d, %d, %d, %d, %d", convertor->dst_sample_rate,
         convertor->src_sample_rate, len, convertor->src_nb_samples, convertor->dst_nb_samples,
         convertor->max_dst_nb_samples);
