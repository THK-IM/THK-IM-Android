...
object PCMTransUtil {
     ...
     * 测试发现这种算法会降低 录制的音量
     * 需要两个音频帧的数据长度相同才可以混合
     * bMulRoadAudios[0] 原始默认的音频
     */
    fun averageMix(bMulRoadAudios: Array<ByteArray>): ByteArray {
        val realMixAudio = bMulRoadAudios[0]

        if (bMulRoadAudios.size == 1)
            return realMixAudio

        /**
         * 保证每一帧数据的长度都是一样的
         */
        for (rw in bMulRoadAudios.indices) {
            if (bMulRoadAudios[rw].size != realMixAudio.size) {
                return realMixAudio
            }
        }

        val row = bMulRoadAudios.size
        val column = realMixAudio.size / 2
        val sMulRoadAudios = Array(row) { ShortArray(column) }
        /** byte --> short
        ...
        for (r in 0 until row) {
            for (c in 0 until column) {
                sMulRoadAudios[r][c] = (bMulRoadAudios[r][c * 2].toInt() and 0xff or
                        (bMulRoadAudios[r][c * 2 + 1].toInt() and 0xff shl 8)).toShort()
            }
        }
        ...
        for (sc in 0 until column) {
            ...
            sr = 0
            while (sr < row) {
                mixVal += sMulRoadAudios[sr][sc].toInt()
                ++sr
            }
     ...
     * 会出现杂音
     */
    fun averageMixSelectShort(bMulRoadAudios: Array<ByteArray>?): ByteArray? {

        if (bMulRoadAudios == null || bMulRoadAudios.isEmpty())
            return null
        var realMixAudio = bMulRoadAudios[0]
        ...
        for (rw in bMulRoadAudios.indices) {
            if (bMulRoadAudios[rw].size < realMixAudio.size) {
                realMixAudio = bMulRoadAudios[rw]
            }
        }
